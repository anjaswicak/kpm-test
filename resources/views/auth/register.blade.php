    ...
    <div class="grid w-full max-w-4xl overflow-hidden rounded-2xl bg-[#1d1d2f] shadow-[0_30px_60px_-30px_rgba(0,0,0,0.7)] md:grid-cols-2">
        <div class="relative hidden md:block">
            ...
            <div class="relative z-10 flex h-full flex-col justify-between p-8 text-white">
                ...
                <div>
                    <p class="text-2xl font-semibold leading-tight">Join Demo Web Kompetisi</p>
                    <p class="mt-2 text-sm text-white/75">Buat akun baru untuk mulai bergabung di ujian.</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 text-white md:p-8">
            ...
            <p class="mt-1 text-sm text-white/70">Silakan lengkapi data untuk mendaftar akun.</p>

            <form method="POST" action="{{ route('register') }}" class="mt-5 space-y-4">
                @csrf

                <div>
                    <label for="name" class="text-xs font-medium text-white/80">Nama lengkap</label>
                ...
                <div>
                    <label for="password_confirmation" class="text-xs font-medium text-white/80">Konfirmasi password</label>
            ...
            <p class="mt-4 text-center text-xs text-white/60">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="font-semibold text-violet-300 hover:text-violet-200">Login sekarang</a>
            </p>
        </div>
    </div>
</x-guest-layout>
